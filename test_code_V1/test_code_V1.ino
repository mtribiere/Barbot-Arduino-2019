...
#define SPEED_MOTOR 255

#define MAX_SERVO 55
#define MIN_SERVO 30


#define SENS1_VE true /* droite */
#define SENS2_VE false /* gauche */

#define SENS1_HO true /* sens montée*/
#define SENS2_HO false  /* sens descente*/
...
const int pump_list [] = {9};
const int mot_DC =10;

const int mot_DC_in3 =6; 
const int mot_DC_in4 =5;
...
BasicStepperDriver stepper_HO(MOTOR_STEPS, motor_stepper[0][1], motor_stepper[0][0]);/* module coté analogie*/
BasicStepperDriver stepper_VE(MOTOR_STEPS, motor_stepper[1][1], motor_stepper[1][0]);/*module coté digital*/
...
void init_pin()
{
 ...
 // pinMode(b_left,INPUT);
  ...
  pinMode(mot_DC , 10);
  analogWrite(mot_DC,SPEED_MOTOR);
  pinMode(motor_stepper[1][0],OUTPUT);
  pinMode(motor_stepper[1][1],OUTPUT);
...
void reset()
{
  ...
  { 
    if(!button_active(fdc_stepper1[1],false))
        make_step(5,stepper_HO,SENS1_HO);
...
void playing()
{
  if(button_active(b_play,false))
  {
    ...
    do
    {
        ...
        if(!button_active(fdc_stepper1[1],false)) 
          make_step(2,stepper_HO,SENS1_HO);
...
void make_step(int number_of_step,BasicStepperDriver stepper,boolean way)
{
  ...
  stepper.move(sens*number_of_step);
}
void open_plier(Servo servo)
{
  for (pos_servo  = MIN_SERVO; pos_servo  <= MAX_SERVO; pos_servo  += 1) { // goes from 0 degrees to 180 degrees
...
{
    for (pos_servo  = MAX_SERVO; pos_servo  >= MIN_SERVO; pos_servo  -= 1) { // goes from 180 degrees to 0 degrees
    servo.write(pos_servo);              // tell servo to go to position in variable 'pos'
    delay(50);
...
void run_pump( boolean sens, boolean RUN)
{
  if(RUN)
   {
      if (sens)
      {
        digitalWrite(mot_DC_in3 , HIGH);
        digitalWrite(mot_DC_in4 , LOW);
        
      }
      else
      {
        digitalWrite(mot_DC_in3 , LOW);
        digitalWrite(mot_DC_in4 , HIGH);
      }
   }
   else
   {
